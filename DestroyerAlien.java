...
public class DestroyerAlien {
    ...
    public DestroyerAlien(Game game, AlienManager manager, Position pos) {
        this.life = ARMOR;
        this.pos = pos;
        this.game = game;
        this.dir = Move.LEFT;
        this.cyclesToMove = game.getLevel().getSpeed();
    ...
    //PARA LA BOMBA 
    public void tryToDropBomb(UCMShip ucmShip) {
        if (pos.getX() == ucmShip.getPosition().getX()) {
            Bomb bomb = new Bomb(new Position(pos.getX(), pos.getY() + 1)); 
            bomb.activate(); 
        }
    }
